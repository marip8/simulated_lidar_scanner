...
// The scanner transform is relative to the coordinate system described at the top of the header file. We need only the
// Forward vector.
...
{
  // Return the 3D coordinate of the scanners location. If the scanner transform has not been set, this value is not
  // valid.
  if (Transform)
  {
...
int vtkLidarScanner::RequestData(vtkInformation* vtkNotUsed(request), vtkInformationVector** inputVector,
...
void vtkLidarScanner::ConstructOutput()
{
  ...
  */
  ...
  // std::cout << "extent: " << extent[0] << " " << extent[1] << " " << extent[2] << " " << extent[3] << " " <<
  ...
  vtkSmartPointer<vtkDoubleArray> coordinateArray = vtkSmartPointer<vtkDoubleArray>::New();
  ...
  vtkSmartPointer<vtkIntArray> validArray = vtkSmartPointer<vtkIntArray>::New();
  ...
  vtkUnsignedCharArray* imageScalars = vtkUnsignedCharArray::SafeDownCast(this->Output->GetPointData()->GetArray("Image"
                                                                                                                 "Scala"
                                                                                                                 "rs"));
  ...
  for (unsigned int phi = 0; phi < this->NumberOfPhiPoints; phi++)
  {
    for (unsigned int theta = 0; theta < this->NumberOfThetaPoints; theta++)
    {
      ...
      if (this->Scan->GetValue(phi, theta)->GetHit())
      {
        ...
        pixel[0] = 255;
        pixel[1] = 255;
        pixel[2] = 255;  // We don't collect color, so set valid pixels to white
...
void vtkLidarScanner::AcquirePoint(const unsigned int thetaIndex, const unsigned int phiIndex)
{
  ...
  //   {
  //     vtkIdType intersect = Scene->GetCell(cellIndex)->IntersectWithLine(ray->GetOrigin(), pointOnRay, tolerance, t,
  //     x, pcoords, cellId); if(intersect)
  //     {
  //       hit = true;
  //       break;
  //     }
...
void vtkLidarScanner::MakeSphericalGrid()
{
  ...
  // This grid will later be traversed and the ray at each position will be cast into the scene to look for valid
  // intersections. (the rays will be transformed using the scanner Transform so that they come from the scanner in its
  // current orientation) This function also initializes all of the vtkLidarPoint's in the Scan grid
...
void vtkLidarScanner::WritePTX(const std::string& filename)
{
  ...
  vtkDoubleArray* coordinateArray = vtkDoubleArray::SafeDownCast(this->Output->GetPointData()->GetArray("CoordinateArra"
                                                                                                        "y"));
...
void vtkLidarScanner::GetOutputMesh(vtkPolyData* const output)
{
  ...
  // Go through all the triangles of the Delaunay triangulation and add them to the 3d polydata if they are shorter than
  // MaxMeshEdgeLength
  cells->InitTraversal();
  while (cells->GetNextCell(npts, pts))
  {
    ...
    // If the rays are stored, point 0 is the scanner position. If the rays are not stored, point 0 is the first scan
    // point
    unsigned int offset;
    if (this->StoreRays)
    {
...
void vtkLidarScanner::GetValidOutputPoints(vtkPolyData* const output)
{
  ...
  if (StoreRays)
  {
    ...
    double OriginNormal[3] = { 0.0, 0.0, 1.0 };  // must be a valid normal so tube filter will work (!!! should change
                                                 // this to the scanner's 'up' vector)
#if VTK_MAJOR_VERSION < 7
    norms->InsertNextTupleValue(OriginNormal);  // have to insert a normal for this new point or it will complain "the
                                                // array is too short"
#else
    norms->InsertNextTypedTuple(OriginNormal);  // have to insert a normal for this new point or it will complain "the
                                                // array is too short"
#endif
  }
  ...
  for (unsigned int thetaCounter = 0; thetaCounter < NumberOfThetaPoints; thetaCounter++)
  {
    for (unsigned int phiCounter = 0; phiCounter < NumberOfPhiPoints; phiCounter++)
    {
      ...
      {
        // If StoreRays is on, we will save a unit distance point in every direction so the rays can be visualized even
        ...
        if (StoreRays)
        {
          ...
          double n[3] = { this->RepresentationLength, 0.0,
                          0.0 };  // the normal of a miss point is not defined, so we set it to an arbitrary (1,0,0)
#if VTK_MAJOR_VERSION < 7
...
void vtkLidarScanner::CreateRepresentation(vtkPolyData* const representation)
{
  ...
  // Moving points topLeft and bottomLeft (left, minThetaAngle) or topRight and bottomRight (right, maxThetaAngle) +/- x
  // does this
  ...
  // Moving points topLeft and topRight (top, maxPhiAngle) or bottomLeft and bottomRight (bottom, minPhiAngle) +/- z
  // does this
  ...
  */
  ...
  // Now that we have built the representation in the default coordinate system, transform it to the current scanner
  // transformation
...
void vtkLidarScanner::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << "Scanner" << std::endl << "-----------" << std::endl;
  if (Transform)
  {
    std::cout << "Transform: " << *(this->Transform) << std::endl
              << "Location: " << this->GetPosition()[0] << " " << this->GetPosition()[1] << " "
  ...
  std::cout << "NumberOfThetaPoints: " << this->NumberOfThetaPoints << std::endl
            ...
            << "MaxPhiAngle: " << this->MaxPhiAngle << std::endl
            << std::endl;
}
